...
use App\Models\HwpAnswer;
use App\Models\HwpCampaign;
use App\Models\HwpKeyApi;
use App\Models\HwpQuestion;
...
//        dd($str);
    ...
    public function saveKeyByIdCam(Request $request, $id_cam)
    {
        $body = json_decode($request->getContent());
        for ($j = 0; $j < count($body); $j++) {
            $key = HwpQuestion::where('key_cha', '=', $body[$j]->key_cha)->where('id_cam', '=', $id_cam)->count();
            if ($key == 0) {
                // lưu url key cha
                HwpQuestion::query()->insert([
                    'tien_to' => $body[$j]->tien_to,
                    'key_cha' => $body[$j]->key_cha,
                    'hau_to' => $body[$j]->hau_to,
                    'id_cam' => $id_cam,
                ]);
            }
    ...
    public function getUrlByIdKey($id_key)
    {
        ...
        $list_url = HwpAnswer::where("id_key", '=', $id_key)->get()->toArray();
        return json_encode($list_url);
    }
    ...
    {
        $list_key = HwpQuestion::all()->toArray();
    ...
    public function xoaKey(HwpQuestion $id)
    {
        $id->delete();
    ...
    {
        $list_key = HwpQuestion::where("id_cam", '=', $id_cam)->get()->toArray();
        // if (!empty($list_key)) {
        ...
        //     }
        // }
        HwpQuestion::where('id_cam', '=', $id_cam)->delete();
    ...
    {
        $list =  HwpQuestion::where('key_cha', 'like', '%' . $name . '%')->get()->toArray();
    ...
    public function xoaURL(HwpAnswer $id)
    {
        $id->delete();
    ...
    {
        HwpAnswer::where('id_key', '=', $id_key)->delete();
    ...
    {
        $key = HwpQuestion::where("id_cam", '=', $id)->get()->toArray();
...
//        $key_in_url = DB::table('hwp_key')
//            ->select("hwp_key.id", 'hwp_key.tien_to', 'hwp_key.ten', 'hwp_key.hau_to', 'hwp_key.url_key_cha',
//                'hwp_key.check', 'hwp_key.id_cam', 'hwp_key.key_con_1', 'hwp_key.key_con_2', 'hwp_key.key_con_3',
//                'hwp_key.key_con_4', 'hwp_key.url_key_con_1', 'hwp_key.url_key_con_2', 'hwp_key.url_key_con_3',
//                'hwp_key.url_key_con_4', 'hwp_key.top_view_1', 'hwp_key.url_top_view_1', 'hwp_key.top_view_2',
//                'hwp_key.url_top_view_2', 'hwp_key.top_view_3', 'hwp_key.url_top_view_3', 'hwp_key.top_view_4',
//                'hwp_key.url_top_view_4', 'hwp_key.top_view_5', 'hwp_key.url_top_view_5', 'hwp_key.ky_hieu', 'hwp_key.id_list_vd')
//                ->where("hwp_key.id_cam", '=', $id)
//                ->whereNotIn('hwp_key.id',$list_id)
//            ->get()->toArray();
    ...
    {
        $key = HwpQuestion::where('id_cam', '=', $id_cam)->get()->toArray();
        return json_encode($key);
    }
...
// }
    ...
    public function saveCam(Request $request)
    {
        $body = json_decode($request->getContent());
        for ($j = 0; $j < count($body); $j++) {
            $cam = HwpCampaign::query()->where('campaign', '=', $body[$j]->campaign)->count();
            if ($cam == 0) {
                // lưu url key
                DB::table("hwp_campaigns")->insert([
                    "campaign" => $body[$j]->campaign,
                    "language" => $body[$j]->language,
                    "check" => 0
                ]);
...
//        DB::table('hwp_spin_word')->where('hwp_spin_word.id_cam', '=', $id_cam)->delete();
    ...
    public function export_js($from, $to, $id_cam)
    {
        ...
        if (!empty($list_key)) {
            foreach ($list_key as $key) {
                // if ($key->check == 1) {
                    $post = HwpAnswer::query()
                        // ->join('hwp_yoast_indexable', 'hwp_yoast_indexable.object_id', '=', 'hwp_posts.id')
                        ->join('hwp_questions', 'hwp_questions.id', '=', 'hwp_answers.id_key')
//                    ->join('hwp_url', 'hwp_url.id', '=', 'hwp_posts.id_url')
                        // ->where('post_title', '!=', '')
...
//         }
    ...
    public function saveWeb(Request $request)
    {
        ...
        // try {
            $body = json_decode($request->getContent());
            for ($j = 0; $j < count($body); $j++) {
                // $url =  HwpAnswer::where('answer', '=', $body[$j]->answer)
                //     ->where('id_key', '=', $body[$j]->id_key)->count();
                // if($url == 0) {
                    HwpAnswer::query()->insert([
                        "answer" => $body[$j]->answer,
                        "id_key" => $body[$j]->id_key,
                        "id_cam" => $body[$j]->id_cam
                    ]);
                // }
            }
            return array(
    ...
    {
        $key = HwpQuestion::where("id", '=', $id)->get()->toArray();
        return json_encode($key);
    }
...
//        japan->france
    ...
    public function getAllKeyYoutube() {
        try {
            $key = DB::table("hwp_key_google")->where("hwp_key_google.type", '=', 'youtube')->get()->toArray();
            return json_encode($key);
        } catch(\Exception $e) {
            return array('code'=>500, 'message' => 'k get được key yt');
        }
    ...
    public function saveKeyGoogle(Request $request) {
        $body = json_decode($request->getContent());
        for($j = 0; $j < count($body); $j++) {
            $key = HwpKeyApi::where("key_api", '=', $body[$j]->key_api)->count();
            if($key == 0) {
                HwpKeyApi::query()->insert([
    ...
    public function deleteKeyGoogle(HwpKeyApi $id_key_gg) {
        $id_key_gg->delete();
        return array(
            'success' => true
        );
    ...
    public function deleteAllKeyYoutube() {
        DB::table("hwp_key_google")->where('type',"youtube")->delete();
        return array(
            'success' => true
        );
    }
